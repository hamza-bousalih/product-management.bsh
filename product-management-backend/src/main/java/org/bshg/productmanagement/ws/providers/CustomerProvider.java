...
import org.bshg.productmanagement.ws.converter.CustomerConverter;
...
@Autowired private CustomerConverter converter;
@GetMapping("/id/{id}")
...
public ResponseEntity<List<CustomerDto>> findAllOptimized() {
var result = service.findAllOptimized();
var resultDto = converter.toDto(result);
return ResponseEntity.ok(resultDto);
}
@GetMapping("/paginated")
public ResponseEntity<Pagination<CustomerDto>> findPaginated(
...
@PostMapping("/all")
public ResponseEntity<List<CustomerDto>> save(@RequestBody List<CustomerDto> dtos) {
...
@PutMapping("/all")
public ResponseEntity<List<CustomerDto>> update(@RequestBody List<CustomerDto> dtos) {
if (dtos == null) return ResponseEntity.status(HttpStatus.CONFLICT).build();
var item = converter.toItem(dtos);
var result = service.update(item);
var resultDto = converter.toDto(result);
...
@DeleteMapping("/all")
public ResponseEntity<List<CustomerDto>> delete(@RequestBody List<CustomerDto> dtos) {
if (dtos == null) return ResponseEntity.status(HttpStatus.CONFLICT).build();
var item = converter.toItem(dtos);
service.delete(item);
